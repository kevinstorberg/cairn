...
def readme_for(identity: ProjectIdentity, *, has_logo: bool, runtime: LocalRuntimeDefaults) -> str:
    ...
    return (
        ...
        f"npm --prefix frontend run dev -- --port {runtime.frontend_port}\n"
        "```\n"
        "\n"
        f"Open the API at `http://127.0.0.1:{runtime.app_port}` and the Vite UI at "
        f"`http://localhost:{runtime.frontend_port}/ui/`.\n\n"
        ...
        f"- Resource generator: `scripts/generate.py`, `docs/GENERATOR.md`, and `{identity.core_path}/generator/`\n"
        ...
        "- Graph runtime: `src/graphs/endpoints.py`, `src/graphs/base.py`, `docs/GRAPHS.md`, and `build_config_summary_graph()`\n"
        "- Jobs and diagnostics: `src/jobs/`, `src/diagnostics/`, `docs/JOBS.md`, and `docs/ADMIN_DEBUG.md`\n"
        "- Optional extensions: `src/extensions/`, `config/default.yaml`, `config/models.py`, and `docs/EXTENSIONS.md`\n"
        "- Production preflight: `src/operations/preflight.py`, `scripts/preflight.py`, and `docs/PREFLIGHT.md`\n"
        "- Frontend conventions: `frontend/package.json`, `frontend/src/App.tsx`, `frontend/vite.config.ts`, `frontend/src/shared/config/`, `frontend/src/shared/api/`, and `docs/FRONTEND.md`\n"
        "- Frontend static serving: `src/frontend/static.py`\n"
    )
